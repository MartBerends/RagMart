...
def count_total_documents(csv_file):
    """Count total valid documents (excluding deleted or missing IDs and non-PDFs)."""
    df = pd.read_csv(csv_file)
    valid_docs = df[(~df["Id"].isna()) & (df["Verwijderd"] != True) & (df["ContentType"] == "application/pdf")]
...
def process_documents(csv_file, batch_size):
    ...
    # Get list of unprocessed document IDs (excluding failed files and non-PDFs)
    document_ids = [
        row["Id"] for _, row in df.iterrows()
        if not pd.isna(row["Id"]) and not is_already_processed(row["Id"]) and row.get("Verwijderd", False) != True and row.get("ContentType") == "application/pdf"
    ]
